namespace Skyline.DataMiner.Library.Protocol.Rates
{
	...
	public class RateCalculator
	{
		...
		private Dictionary<string, Common.Interfaces.DuplexStatus> GetDuplexStatus(SLProtocol protocol)
		{
			...
			Dictionary<string, Common.Interfaces.DuplexStatus> duplexStatus = new Dictionary<string, Common.Interfaces.DuplexStatus>();
			...
			if (columns.Length == 2 && ((object[])columns[0]).Length == ((object[])columns[1]).Length)
			{
				...
				{
					duplexStatus[Convert.ToString(pkeyColumn[i])] = (Common.Interfaces.DuplexStatus)Convert.ToInt32(duplexStateColumn[i]);
				}
			}
		...
		private void GetData(SLProtocol protocol, TimeSpan? delta, int minDelta, bool utilization, bool discontinuity)
		{
			...
			for (int i = 0; i < interfaceColumns.PKs.Length; i++)
			{
				...
				else
				{
					...
					speedValue = -1;
					duplexStatus = DuplexStatus.NotInitialized;
				}
		...
		private void GetDataUtilization(SLProtocol protocol, uint uiSpeedValue, string sPK, out double speedValue, out DuplexStatus duplexStatus)
		{
			if (speedType == SpeedType.Low)
			{
			...
			Dictionary<string, DuplexStatus> duplexValues = GetDuplexStatus(protocol);
