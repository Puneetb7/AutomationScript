namespace Skyline.DataMiner.Library.Protocol.Matrix
{
	...
	public class MatrixHelper
	{
		...
		internal MatrixHelper(SLProtocol protocol, MatrixHelperParameterIds matrixHelperParameters, int maxInputCount, int maxOutputCount)
		{
			...
			ParseProtocolInfo(elementProtocolResponseMessage, matrixHelperParameters, foundParameterIds); //determs the max size 
			...
			MatrixSizeInfo maxSize;

			if (MaxInputs != -1 && MaxOutputs != -1)
			{
				maxSize = new MatrixSizeInfo(MaxInputs, MaxOutputs);
			}
			else
			{
				maxSize = new MatrixSizeInfo(-1, -1, true);
			}
			...
			else if (matrixHelperParameters.MatrixDummyReadParameterId > 0)
			{
				...
				if (maxFoundSize.IsDynamic)
				{
					MaxInputs = maxFoundSize.Inputs;
					MaxOutputs = maxFoundSize.Outputs;
				}
				displayedInputs = maxFoundSize.Inputs + 1;
				displayedOutputs = maxFoundSize.Outputs + 1;
			}
		...
		protected virtual void OnToolTipSetFromUI(MatrixToolTipSetFromUIMessage set)
		{
		}
		...
		protected virtual void OnPageSetFromUI(MatrixPageSetFromUIMessage set)
		{
		}
		...
		/// </summary>
		/// <param name="set">Information about the changed crosspoint connections.</param>
		protected virtual void OnCrossPointsSetViaLockOverrideFromUI(MatrixCrossPointsSetFromUIMessage set)
		{
		}
			...
			else
			{
				switch (type)
				{
					...
					case "IsEnabled":
						...
						break;
					case "Page":
						setCol[count] = matrixPort.Page;
						break;
					case "ToolTip":
						var output = matrixPort as MatrixOutput;
						if (output != null)
						{
							setCol[count] = output.ToolTip;
						}
						break;
					default:
						return;
		...
		private void ApplyChangesNoDisplayTypeChanged(SLProtocol protocol)
		{
			...
			else if (displayType == MatrixDisplayType.Tables)
			{
				...
				Dictionary<string, string> allChangedInputPages = new Dictionary<string, string>();
				...
				Dictionary<string, string> allChangedTooltips = new Dictionary<string, string>();
				Dictionary<string, string> allChangedOutputPages = new Dictionary<string, string>();
				...
				portState.ToolTips.GetChangedTableItems(allChangedTooltips);
				portState.InputPages.GetChangedTableItems(allChangedInputPages);
				portState.OutputPages.GetChangedTableItems(allChangedOutputPages);

				isTableChanged = ApplyTableChanges(protocol, allChangedInputLabels, allChangedOutputLabels, allChangedInputLocks, allChangedOutputLocks, allChangedInputStates, allChangedOutputStates, allChangedTooltips, allChangedInputPages, allChangedOutputPages);
			}
		...
		private void ApplyChangesDisplayTypeChanged(SLProtocol protocol)
		{
			...
			else
			{
				...
				else
				{
					...
					portState.InputStates.ExecuteChangedItems();
					portState.OutputStates.ExecuteChangedItems();
					//portState.ToolTips.ExecuteChangedItems();
					GetChangedConnections(protocol);
					ApplyTableFullSet(protocol);
				}
			}
		...
		private bool VerifyInputWriteParameterValue(int writeParameterId, int key, string setValue)
		{
			...
			if (writeParameterId == inputTableInfo.PageWriteParameterId)
			{
				if (inputs[key].Page != setValue)
				{
					OnPageSetFromUI(new MatrixPageSetFromUIMessage(key, MatrixIOType.Input, setValue));
				}
				return true;
			}
		...
		private bool VerifyOutputWriteParameterValue(int writeParameterId, int key, string setValue)
		{
			...
			if (writeParameterId == outputTableInfo.LockOverrideWriteParameterId)
			{
				ProcessConnectedWriteParameterValue(key, setValue, true);
				return true;
			}

			if (writeParameterId == outputTableInfo.PageWriteParameterId)
			{
				if (outputs[key].Page != setValue)
				{
					OnPageSetFromUI(new MatrixPageSetFromUIMessage(key, MatrixIOType.Output, setValue));
				}
				return true;
			}

			if (writeParameterId == outputTableInfo.ToolTipWriteParameterId)
			{
				if (outputs[key].ToolTip != setValue)
				{
					OnToolTipSetFromUI(new MatrixToolTipSetFromUIMessage(key, setValue));
				}
				return true;
			}
		...
		private void ProcessConnectedWriteParameterValue(int output, string setValue, bool isViaLockOverride = false)
		{
			...
			if (!isValidInput || input > displayedInputs || (outputs[output].IsLocked && !isViaLockOverride) || !outputs[output].IsEnabled)
			{
				return;
			}
			...
			{
				input--;
				if (connectedInputs.Contains(input) || (inputs[input].IsLocked && !isViaLockOverride) || !inputs[input].IsEnabled)
				{
			...
			DisconnectConnectedInputs(connectedInputs, output, updatedCrossPoints, isViaLockOverride);

			if (updatedCrossPoints.Any())
			{
				if (!isViaLockOverride)
				{
					OnCrossPointsSetFromUI(new MatrixCrossPointsSetFromUIMessage(updatedCrossPoints));
				}
				else
				{
					OnCrossPointsSetViaLockOverrideFromUI(new MatrixCrossPointsSetFromUIMessage(updatedCrossPoints));
				}
			}
		}

		private void DisconnectConnectedInputs(ICollection<int> connectedInputs, int output, ICollection<MatrixCrossPointSetFromUI> updatedCrossPoints, bool isViaLockOverride)
		{
			...
			foreach (int connectedInput in connectedInputs)
			{
				...
				{
					if (inputs[connectedInput].IsLocked && !isViaLockOverride)
					{
		...
		private bool ApplyTableChanges(SLProtocol protocol, Dictionary<string, string> allChangedInputLabels, Dictionary<string, string> allChangedOutputLabels, Dictionary<string, string> allChangedInputLocks, Dictionary<string, string> allChangedOutputLocks, Dictionary<string, string> allChangedInputStates, Dictionary<string, string> allChangedOutputStates, Dictionary<string, string> allChangedTooltips = null, Dictionary<string, string> allChangedInputPages = null, Dictionary<string, string> allChangedOutputPages = null)
		{
			if (allChangedTooltips == null)
			{ allChangedTooltips = new Dictionary<string, string>(); }
			if (allChangedInputPages == null)
			{ allChangedInputPages = new Dictionary<string, string>(); }
			if (allChangedOutputPages == null)
			{ allChangedOutputPages = new Dictionary<string, string>(); }
			...
			bool isTableChanged = false;
			bool isInputItemChanged = allChangedInputLabels.Any() || allChangedInputLocks.Any() || allChangedInputStates.Any() || allChangedInputPages.Any();
			if (isInputItemChanged || changeMatrixSize)
			{
				ApplyTableChangesInput(protocol, allChangedInputLabels, allChangedInputLocks, allChangedInputStates, allChangedInputPages);
				isTableChanged = true;
			}

			bool isOutputItemChanged = allChangedOutputLabels.Any() || allChangedOutputLocks.Any() || allChangedOutputStates.Any() || allChangedOutputPages.Any() || allChangedTooltips.Any() || portState.Connections.UpdatedConnections.Any();
			if (isOutputItemChanged || changeMatrixSize)
			{
				ApplyTableChangesOutput(protocol, allChangedOutputLabels, allChangedOutputLocks, allChangedOutputStates, allChangedTooltips, allChangedOutputPages);
				isTableChanged = true;
			}
		...
		private void ApplyTableChangesInput(SLProtocol protocol, Dictionary<string, string> allChangedInputLabels, Dictionary<string, string> allChangedInputLocks, Dictionary<string, string> allChangedInputStates, Dictionary<string, string> allChangedInputPages = null)
		{
			if (changeMatrixSize)
			{
				ApplyTableChangesInputSizeChange(protocol, allChangedInputLabels, allChangedInputLocks, allChangedInputStates);
			}
			bool hasPage = allChangedInputPages != null && outputTableInfo.PageColumnIdx > 0;
			...
			if (hasPage)
			{ GetNeededKeys(neededInputKeys, allChangedInputPages, displayedOutputs); }
			...
			object[] setInputPageCol = new object[neededInputKeys.Count];
			int count = 0;
			foreach (string key in neededInputKeys)
			{
				...
				if (hasPage)
				{ SetColumnValue(key, count, matrixPort, setInputPageCol, allChangedInputPages, "Page"); }
				count++;
			}

			List<object> setTable = new List<object>();
			setTable.Add(setInputKeyCol);
			setTable.Add(setInputLabelCol);
			setTable.Add(setInputStateCol);
			setTable.Add(setInputLockCol);
			if (hasPage)
			{ setTable.Add(setInputPageCol); }

			List<object> parameterIds = new List<object>();
			parameterIds.Add(inputTableInfo.TableParameterId);
			parameterIds.Add(inputTableInfo.LabelParameterId);
			parameterIds.Add(inputTableInfo.EnabledParameterId);
			parameterIds.Add(inputTableInfo.LockedParameterId);
			if (hasPage)
			{ parameterIds.Add(inputTableInfo.PageParameterId); }
		...
		private void ApplyTableChangesOutput(SLProtocol protocol, Dictionary<string, string> allChangedOutputLabels, Dictionary<string, string> allChangedOutputLocks, Dictionary<string, string> allChangedOutputStates, Dictionary<string, string> allChangedTooltips, Dictionary<string, string> allChangedOutputPages)
		{
			if (changeMatrixSize)
			{
				ApplyTableChangesOutputSizeChange(protocol, allChangedOutputLabels, allChangedOutputLocks, allChangedOutputStates);
			}
			bool hasToolTip = allChangedTooltips != null && outputTableInfo.ToolTipColumnIdx > 0;
			bool hasPage = allChangedOutputPages != null && outputTableInfo.PageColumnIdx > 0;
			...
			if (hasToolTip)
			{ GetNeededKeys(neededOutputKeys, allChangedTooltips, displayedOutputs); }
			if (hasPage)
			{ GetNeededKeys(neededOutputKeys, allChangedOutputPages, displayedOutputs); }
			...
			object[] setOutputPageCol = new object[neededOutputKeys.Count];
			object[] setOutputToolTipCol = new object[neededOutputKeys.Count];
			...
			foreach (string key in neededOutputKeys)
			{
				...
				if (hasToolTip)
				{ SetColumnValue(key, count, matrixPort, setOutputToolTipCol, allChangedTooltips, "ToolTip"); }
				if (hasPage)
				{ SetColumnValue(key, count, matrixPort, setOutputPageCol, allChangedOutputPages, "Page"); }
			...
			List<object> setTable = new List<object>();
			setTable.Add(setOutputKeyCol);
			setTable.Add(setOutputLabelCol);
			setTable.Add(setOutputStateCol);
			setTable.Add(setOutputLockCol);
			setTable.Add(setOutputConnectedInputCol);
			if (hasToolTip)
			{ setTable.Add(setOutputToolTipCol); }
			if (hasPage)
			{ setTable.Add(setOutputPageCol); }

			List<object> parameterIds = new List<object>();
			parameterIds.Add(outputTableInfo.TableParameterId);
			parameterIds.Add(outputTableInfo.LabelParameterId);
			parameterIds.Add(outputTableInfo.EnabledParameterId);
			parameterIds.Add(outputTableInfo.LockedParameterId);
			parameterIds.Add(outputTableInfo.ConnectedParameterId);
			if (hasToolTip)
			{ parameterIds.Add(outputTableInfo.ToolTipParameterId); }
			if (hasPage)
			{ parameterIds.Add(outputTableInfo.PageParameterId); }
		...
		private void ParseProtocolManualTables(ParameterInfo[] parameters, MatrixHelperParameterIds requestParameters, MatrixHelperParameterIds foundParameters, Dictionary<int, uint> inputColumns, Dictionary<int, uint> outputColumns, MatrixHelperParameterNames matrixHelperParameterNames)
		{
			...
			MatrixCustomTableInfoItem inputPage = new MatrixCustomTableInfoItem(true);
			...
			MatrixCustomTableInfoItem outputPage = new MatrixCustomTableInfoItem(false);
			MatrixCustomTableInfoItem outputTooltip = new MatrixCustomTableInfoItem(false);
			MatrixCustomTableInfoItem outputLockOverride = new MatrixCustomTableInfoItem(false);

			var inputLabelMap = GetMatrixMapItemBasedOnName(requestParameters.InputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_LABEL);
			var inputIsEnabledMap = GetMatrixMapItemBasedOnName(requestParameters.InputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_STATE);
			var inputIsLockedMap = GetMatrixMapItemBasedOnName(requestParameters.InputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_LOCK);
			var inputPageMap = GetMatrixMapItemBasedOnName(requestParameters.InputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_PAGE);

			var ouputLabelMap = GetMatrixMapItemBasedOnName(requestParameters.OutputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_LABEL);
			var outputIsEnabledMap = GetMatrixMapItemBasedOnName(requestParameters.OutputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_STATE);
			var outputIsLockedMap = GetMatrixMapItemBasedOnName(requestParameters.OutputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_LOCK);
			var outputConnectedInputMap = GetMatrixMapItemBasedOnName(requestParameters.OutputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_CONNECTED_INPUT);
			var outputPageMap = GetMatrixMapItemBasedOnName(requestParameters.OutputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_PAGE);
			var outputToolTipMap = GetMatrixMapItemBasedOnName(requestParameters.OutputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_TOOLTIP);
			var outputLockOverrideMap = GetMatrixMapItemBasedOnName(requestParameters.OutputMappings, MatrixHelperParameterNames.MATRIX_MAPPING_NAME_LOCK_OVERRIDE);
			...
			for (int i = 0; i < parameters.Length; i++)
			{
				...
				inputLabel.ProcessParameter(parameter, matrixHelperParameterNames.InputsLabelParameterName, isParameterString, inputColumns, inputLabelMap);
				inputIsEnabled.ProcessParameter(parameter, matrixHelperParameterNames.InputsIsEnabledParameterName, isParameterDiscreet, inputColumns, inputIsEnabledMap);
				inputIsLocked.ProcessParameter(parameter, matrixHelperParameterNames.InputsIsLockedParameterName, isParameterDiscreet, inputColumns, inputIsLockedMap);
				inputPage.ProcessParameter(parameter, matrixHelperParameterNames.InputsPageParameterName, isParameterString, inputColumns, inputPageMap);
				outputLabel.ProcessParameter(parameter, matrixHelperParameterNames.OutputsLabelParameterName, isParameterString, outputColumns, ouputLabelMap);
				outputIsEnabled.ProcessParameter(parameter, matrixHelperParameterNames.OutputsIsEnabledParameterName, isParameterDiscreet, outputColumns, outputIsEnabledMap);
				outputIsLocked.ProcessParameter(parameter, matrixHelperParameterNames.OutputsIsLockedParameterName, isParameterDiscreet, outputColumns, outputIsLockedMap);
				outputPage.ProcessParameter(parameter, matrixHelperParameterNames.OutputPageParameterName, isParameterString, outputColumns, outputPageMap);
				outputConnectedInput.ProcessParameter(parameter, matrixHelperParameterNames.OutputsConnectedInputParameterName, isParameterString, outputColumns, outputConnectedInputMap);
				outputTooltip.ProcessParameter(parameter, matrixHelperParameterNames.OutputsToolTipParameterName, isParameterString, outputColumns, outputToolTipMap);
				outputLockOverride.ProcessParameter(parameter, matrixHelperParameterNames.OutputsLockOverrideParameterName, isParameterString, outputColumns, outputLockOverrideMap);

				string parameterName = parameter.Name.ToLower(CultureInfo.InvariantCulture);
				if ((parameterName == matrixHelperParameterNames.OutputsTableVirtualSetParameterName || requestParameters.TableVirtualSetParameterId == parameter.ID) && isParameterString && parameter.WriteType)
				{
					foundParameters.TableVirtualSetParameterId = parameter.ID;
				}
				else if ((parameterName == matrixHelperParameterNames.OutputsTableSerializedWritesParameterName || requestParameters.TableSerializedParameterId == parameter.ID) && isParameterString && parameter.WriteType)
				{
			...
			//inputPage.ValidateParameters(); // do not check since this one is optional
			//outputPage.ValidateParameters(); // do not check since this one is optional
			//outputTooltip.ValidateParameters(); // do not check since this one is optional
			//outputLockOverride.ValidateParameters(); // do not check since this one is optional
			...
			AssignCustomNumbersForTable(requestParameters);
			inputTableInfo = new MatrixCustomTableInfo(requestParameters.InputsTableParameterId, inputLabel, inputIsEnabled, inputIsLocked, inputIsConnected, MaxInputs, true, pageId: inputPage);
			outputTableInfo = new MatrixCustomTableInfo(requestParameters.OutputsTableParameterId, outputLabel, outputIsEnabled, outputIsLocked, outputConnectedInput, MaxOutputs, false, pageId: outputPage, toolTipId: outputTooltip, lockOverrideId: outputLockOverride);
		}

		private MatrixMap GetMatrixMapItemBasedOnName(List<MatrixMap> mapping, string name)
		{
			if (!String.IsNullOrEmpty(name) && mapping != null)
			{
				var map = mapping.FirstOrDefault(item => item.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase));
		...
		private void AssignCustomNumbersForTable(MatrixHelperParameterIds requestParameters)
		{
			if (requestParameters.MatrixReadParameterId <= 0 && requestParameters.MatrixReadParameterId != -2)
			{
				...
				else if (requestParameters.MatrixDummyReadParameterId > 0) // dynamic matrix (follows tables size)
				{
					MaxInputs = -1;
					MaxOutputs = -1;
					displayedInputs = -1;
					...
					//defaults, will be overriden via the TableMatrixHelper => SetMatrixOptions if needed
					MinConnectedInputsPerOutput = 0;
					MaxConnectedInputsPerOutput = 1;
					MinConnectedOutputsPerInput = 0;
					MaxConnectedOutputsPerInput = -2;
				}
				else //not a dynamic table matrix
				{
