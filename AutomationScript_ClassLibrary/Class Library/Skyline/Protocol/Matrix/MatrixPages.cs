namespace Skyline.DataMiner.Library.Protocol.Matrix
{
	...
	internal class MatrixPages : MatrixItems<string>
	{

		internal MatrixPages(MatrixPortState portState, MatrixIOType type, IDictionary<int, string> pageValues) : base(portState, type)
		{
			foreach (KeyValuePair<int, string> kvp in pageValues)
			{
				OriginalItems[kvp.Key] = kvp.Value;
			}
		}
		/// <summary>
		/// Gets or sets the page of a input / output.
		/// </summary>
