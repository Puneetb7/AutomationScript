namespace Skyline.DataMiner.Library.Protocol.Matrix
{
	...
	internal class MatrixCustomTableInfoItem
	{
		...
		internal void ProcessParameter(ParameterInfo parameter, string requestedName, bool isValidType, IDictionary<int, uint> columns, MatrixMap map)
		{
			bool isMatchingMap = map != null && (map.TableColumnPid == parameter.ID || (parameter.WriteType && map.TableColumnWritePid == parameter.ID));
			if (!isMatchingMap && (!isValidType || (parameter.Name.ToLower(CultureInfo.InvariantCulture) != requestedName))) return;
