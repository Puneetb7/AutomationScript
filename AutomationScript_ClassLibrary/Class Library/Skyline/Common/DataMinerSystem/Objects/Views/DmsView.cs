namespace Skyline.DataMiner.Library.Common
{
	...
	internal class DmsView : DmsObject, IDmsView
	{
		...
		/// <summary>
		/// The services that are part of this view.
		/// </summary>
		private readonly List<IDmsService> services = new List<IDmsService>();
		...
		private bool areServicesLoaded;
		...
		/// <summary>
		/// Gets all services contained in this view.
		/// </summary>
		/// <value>The services contained in this view.</value>
		public IList<IDmsService> Services
		{
			get
			{
				if (!areServicesLoaded)
				{
					LoadServicesOnDemand();
				}
		...
		/// <summary>
		/// Loads the elements in the view.
		/// </summary>
		private void LoadElementsOnDemand()
		{
			try
			{
				areElementsLoaded = true;
				GetLiteElementInfo message = new GetLiteElementInfo
				{
					ViewID = Id,
				...
				foreach (DMSMessage response in responses)
				{
					LiteElementInfoEvent elementInfo = (LiteElementInfoEvent)response;
					DmsElement element = new DmsElement(this.dms, new DmsElementId(elementInfo.DataMinerID, elementInfo.ElementID));
		...
		/// <summary>
		/// Loads the services in the view.
		/// </summary>
		private void LoadServicesOnDemand()
		{
			try
			{
				areServicesLoaded = true;
				GetLiteServiceInfo message = new GetLiteServiceInfo
				{
					ViewID = Id,
					ExcludeSubViews = true,
				};
				...
				foreach (DMSMessage response in responses)
				{
					LiteServiceInfoEvent serviceInfo = (LiteServiceInfoEvent)response;
					DmsService service = new DmsService(this.dms, new DmsServiceId(serviceInfo.DataMinerID, serviceInfo.ID));
